...
const HaalandArticle: React.FC<HaalandArticleProps> = ({ content }) => {
  ...
  const options: HTMLReactParserOptions = {
    replace: (domNode: DOMNode) => {
      ...
      if (name === "figure") return <Figure>{domToReact(children as DOMNode[], options)}</Figure>;
      if (name === "img") return <Img attribs={attribs} />;
      if (/^h[1-6]$/.test(name)) {
        const level = parseInt(name.replace("h", ""), 10);
        return <Heading level={level}>{domToReact(children as DOMNode[], options)}</Heading>;
      }
      if (name === "p") return <p className="text-base leading-relaxed my-3">{domToReact(children as DOMNode[], options)}</p>;
      if (name === "a") {
        return (
          <a
            href={attribs.href}
